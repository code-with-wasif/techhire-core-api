...
public class JobPostSvc {
    ...
    public ResponseEntity<HttpStatus> changePostEntry(JobPostDto newEntry, String id){
        ...
        try {
            ...
            if (newEntry.getMinSalary() != null && newEntry.getMinSalary() >= 0) oldEntry.setMinSalary(newEntry.getMinSalary());
            if (newEntry.getMaxSalary() != null && newEntry.getMaxSalary() >= 0) oldEntry.setMaxSalary(newEntry.getMaxSalary());
            if (newEntry.getTechStack() != null && !newEntry.getTechStack().isEmpty()) oldEntry.setTechStack(newEntry.getTechStack());
            if (StringUtils.hasText(newEntry.getStatus())) oldEntry.setStatus(newEntry.getStatus());
            jobPostRepo.save(oldEntry);
            return new ResponseEntity<>(HttpStatus.OK);
        } catch (Exception e) {
            log.error("Error while making changes", e);
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
    }
